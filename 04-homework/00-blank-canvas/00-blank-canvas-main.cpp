...
int main(int, char**)
{
    ...
    color new_color = green.invert_color(); //invert the green color which is purple/magenta

    canvas inverted_image(width, height); //TODO ask about code refactoring, problem: code doubling

    std::fill(inverted_image.begin(), inverted_image.end(), new_color); //fill the canvas with the new color
