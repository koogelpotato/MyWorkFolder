...
int main(int argc, char **argv) {
  ...
  v0[0] /= v0[2], v0[1] /= v0[2]; // convert 3D into 2D with perspective
                                  // division
  v1[0] /= v1[2], v1[1] /= v1[2];
  v2[0] /= v2[2], v2[1] /= v2[2];
  v0[0] = (1 + v0[0]) * 0.5 * w,
  v0[1] = (1 + v0[1]) * 0.5 *
          h; // scale the image according to the device and raster it
  ...
  for (uint32_t j = 0; j < h; ++j) {
    for (uint32_t i = 0; i < w; ++i) {
      ...
      if (w0 >= 0 && w1 >= 0 && w2 >= 0) { // check if color is inside triangle
        w0 /= area;                        // normalize weights
        w1 /= area;
        w2 /= area;
        float r = w0 * c0[0] + w1 * c1[0] +
                  w2 * c2[0]; // calculate color value by using color weights
        float g = w0 * c0[1] + w1 * c1[1] + w2 * c2[1];
        float b = w0 * c0[2] + w1 * c1[2] + w2 * c2[2];
        framebuffer[j * w + i][0] =
            (unsigned char)(r * 255); // assign pixel value
